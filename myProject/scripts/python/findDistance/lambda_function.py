...
def lambda_handler(event, context):
    ...
    try:
        ...
        # print(response['Summary'])
            ...
            #         if i != j:
            #             print("start coordinates: " + str(coords[i][0][0]) + " " + str(coords[i][0][1]))
            #             print("end coordinates: " + str(coords[j][0][0]) + " " + str(coords[j][0][1]))
            #             # print(round(ST_DistanceSphere(airports1.location, airports2.location) / 1000, 0))
            #             distance = calcd(coords[i][0][0],coords[i][0][1], coords[j][0][0], coords[j][0][1] )
            #             print(distance)
            #             # response = location.calculate_route(
            #             #     CalculatorName=calculator['CalculatorName'],
            #             #     DeparturePosition=[round(coords[i][0][0], 6), round(coords[i][0][1], 6)],
            #             #     DestinationPosition=[round(coords[j][0][0], 6), round(coords[j][0][1], 6)],
            #             #     IncludeLegGeometry=False,
            #             #     DistanceUnit='Kilometers'
            #             # )
            #             distances.append((distance, (coords[i][1], coords[j][1])))
